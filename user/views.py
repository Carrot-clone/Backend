    ...
    def post(self, request):
        try:
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                return Response({"msg" : "회원가입에 성공하셨습니다"}, status=status.HTTP_202_ACCEPTED)
        except IntegrityError:
            return Response({"msg" : "회원가입에 실패하셨습니다"}, status=status.HTTP_400_BAD_REQUEST)
...
class UserLoginView(APIView):
    ...
    def post(self, request):
        ...
        if not serializer.is_valid(raise_exception=True):
            return Response({"msg": "로그인에 실패했습니다.","status" : 400}, status=status.HTTP_400_BAD_REQUEST)
